...
COLUMNAS = 10
MAR = " "
PORTAAVIONES = "P"  # Ocupa 4 casillas
ACORAZADO = "A" #Ocupa 3 casillas
CRUCERO = "C" #Ocupa 2 casillas
DESTRUCTOR = "D"  # Ocupa 1 casilla
DISPARO_FALLADO = "-"
DISPARO_ACERTADO = "*"
...
def colocar_e_imprimir_barcos(matriz, cantidad_barcos, jugador):
    ...
    barcos_una_celda = cantidad_barcos//4 #destructor
    barcos_dos_celdas_verticales = cantidad_barcos//9 #crucero
    barcos_tres_celdas_horizontales = cantidad_barcos//9 #acorazado
    barcos_cuatro_celdas_verticales = cantidad_barcos//9 #portaaviones
    if jugador == JUGADOR_1:
        print("Imprimiendo barcos del jugador 1 ")
    else:
        print("Imprimiendo barcos del jugador 2 ")
    print(f"Barcos de una celda: {barcos_una_celda}\nBarcos verticales de dos celdas: {barcos_dos_celdas_verticales}\nBarcos horizontales de tres celdas: {barcos_tres_celdas_horizontales}\nBarcos verticales de cuatro celdas: {barcos_cuatro_celdas_verticales}\nTotal: {barcos_una_celda+barcos_dos_celdas_verticales+barcos_tres_celdas_horizontales+barcos_cuatro_celdas_verticales}")
    # Acomodar el orden de los barcos
    matriz = colocar_barcos_de_cuatro_celdas_verticales(
        barcos_cuatro_celdas_verticales,PORTAAVIONES, matriz)
    matriz = colocar_barcos_de_tres_celdas_horizontales(
        barcos_tres_celdas_horizontales,ACORAZADO, matriz)
    matriz = colocar_barcos_de_dos_celdas_vertical(
        barcos_dos_celdas_verticales, CRUCERO, matriz)
    matriz = colocar_barcos_de_una_celda(barcos_una_celda, DESTRUCTOR, matriz)
    return matriz
...
#DESARROLLO DE LAS FUNCIONES COLOCAR.....
...
def colocar_barcos_de_tres_celdas_horizontales(cantidad, tipo_barco, matriz):
    barcos_colocados = 0
    while barcos_colocados < cantidad:
        ...
        x2 = x + 1
        x3 = x + 2
        if (coordenada_en_rango(x, y) and
            coordenada_en_rango(x2, y) and
            coordenada_en_rango(x3, y) and
            es_mar(x, y, matriz) and
            es_mar(x2, y, matriz) and
            es_mar(x3, y, matriz)):
...
def colocar_barcos_de_cuatro_celdas_verticales(cantidad, tipo_barco, matriz):
    barcos_colocados = 0
    while barcos_colocados < cantidad:
        x = obtener_x_aleatoria()
        y = obtener_y_aleatoria()
        ...
        y4 = y + 3
        if (coordenada_en_rango(x, y) and
            coordenada_en_rango(x, y2) and
            coordenada_en_rango(x, y3) and
            coordenada_en_rango(x, y4) and
            es_mar(x, y, matriz) and
            es_mar(x, y2, matriz) and
            es_mar(x, y3, matriz) and
            es_mar(x, y4, matriz)):
...
def instrucciones():
    print("Instrucciones \n Bienvenido al juego batalla naval \n el juego consiste en dos tableros para dos jugadores \n Cada jugador tendra en su tablero: 1 Portaaviones, 1 Acorazado, 1 Crucero y 2 Destructores \n divididos en todo el tablero \n Indica donde deseas disparar para destruir los barcos de tu contrincante indicando las coordenadas \n letras de fila - numero de colummna \n¡EXITOS!")
